...
"""
text = text.replace("from unittest.mock import AsyncMock, MagicMock, patch, mock_open", patch_lines)

# Fix test_expired
text = text.replace("mock_interaction.response.send_message.assert_called_once()\n        assert \"expired\" in mock_interaction.response.send_message.call_args[0][0]\n        mock_interaction.message.delete.assert_called_once()", "mock_interaction.edit_original_response.assert_called_once()\n        assert \"expired\" in mock_interaction.edit_original_response.call_args[1][\"content\"]")

# Fix test_expired_delete_exception
text = text.replace("mock_interaction.response.send_message.assert_called_once()", "mock_interaction.edit_original_response.assert_called_once()")
...
text = text.replace("mock_interaction.followup.send.assert_called_once()", "mock_interaction.edit_original_response.assert_called_once()")
text = text.replace("kwargs = mock_interaction.followup.send.call_args[1]", "kwargs = mock_interaction.edit_original_response.call_args[1]")
