...
def mock_db_and_broadcast():
    with patch("mods.revocord.hunting.update_encounter_broadcast", new_callable=AsyncMock), \
         ...
         patch("scripts.gradexDB.active_spawns_table.get_spawn", new_callable=AsyncMock, return_value={"mock": "data"}), \
...
class TestOnInteraction:
    ...
    @pytest.mark.asyncio
    async def test_ignore_wrong_type(self, cog: HuntingCog, mock_interaction: Any) -> None:
    ...
    async def test_return_console(self, mock_get_account: Any, cog: HuntingCog, mock_interaction: Any) -> None:
    ...
    async def test_wrong_user(self, cog: HuntingCog, mock_interaction: Any) -> None:
        ...
        assert "Only the trainer" in mock_interaction.response.send_message.call_args[0][0]

    @pytest.mark.asyncio
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_expired(self, mock_get_account: Any, cog: HuntingCog, mock_interaction: Any) -> None:
    ...
    @pytest.mark.asyncio
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_run(self, mock_get_account: Any, cog: HuntingCog, mock_interaction: Any) -> None:
        mock_get_account.return_value = {}
        mock_interaction.type = discord.InteractionType.component
    ...
    @pytest.mark.asyncio
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_catch_menu_no_orbs(self, mock_get_account: Any, cog: HuntingCog, mock_interaction: Any) -> None:
        mock_get_account.return_value = {"inventory": {}}
        mock_interaction.type = discord.InteractionType.component
        timestamp = int(time.time())
    ...
    @pytest.mark.asyncio
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_catch_menu_with_orbs(self, mock_get_account: Any, cog: HuntingCog, mock_interaction: Any) -> None:
    ...
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_throw_orb_success(self, mock_get_account: Any, mock_update: Any, cog: HuntingCog, mock_interaction: Any, sample_revomon: Any) -> None:
    ...
    @patch("mods.revocord.hunting.update_account", new_callable=AsyncMock)
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_throw_orb_fail_flee(self, mock_get_account: Any, mock_update: Any, cog: HuntingCog, mock_interaction: Any, sample_revomon: Any) -> None:
        mock_get_account.return_value = {"inventory": {"159": 2}}
        cog.revomons = [sample_revomon]
        mock_interaction.type = discord.InteractionType.component
        timestamp = int(time.time())
    ...
    @patch("mods.revocord.hunting.update_account", new_callable=AsyncMock)
    @patch("mods.revocord.hunting.get_or_create_account", new_callable=AsyncMock)
    async def test_spawn_throw_orb_fail_no_flee(self, mock_get_account: Any, mock_update: Any, cog: HuntingCog, mock_interaction: Any, sample_revomon: Any) -> None:
        mock_get_account.return_value = {"inventory": {"159": 2}}
        cog.revomons = [sample_revomon]
        mock_interaction.type = discord.InteractionType.component
        timestamp = int(time.time())
    ...
    @pytest.mark.asyncio
    async def test_handle_wilds_claim(self, cog: HuntingCog, mock_interaction: Any) -> None:
