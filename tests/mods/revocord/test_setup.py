...
class TestSetupCogCommand:
    ...
    @pytest.mark.asyncio
    async def test_missing_roles_permission(
        self, setup_cog: Any, mock_interaction: Any
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_missing_channels_permission(
        self, setup_cog: Any, mock_interaction: Any
    ) -> None:
        ...
        mock_interaction.followup.send.assert_called_once()
        assert (
            "manage channels" in mock_interaction.followup.send.call_args[0][0].lower()
        )

    @pytest.mark.asyncio
    async def test_missing_messages_permission(
        self, setup_cog: Any, mock_interaction: Any
    ) -> None:
        mock_interaction.app_permissions.manage_roles = True
        ...
        mock_interaction.followup.send.assert_called_once()
        assert (
            "manage messages" in mock_interaction.followup.send.call_args[0][0].lower()
        )
    ...
    @pytest.mark.asyncio
    async def test_sends_biome_view(
        self, setup_cog: Any, mock_interaction: Any
    ) -> None:
        mock_interaction.app_permissions.manage_roles = True
        mock_interaction.app_permissions.manage_channels = True
        ...
        assert isinstance(
            mock_interaction.followup.send.call_args[1]["view"], BiomeSelectView
        )
    ...
    async def test_setup_full_creation(
        self, mock_spawn: Any, mock_sleep: Any, setup_cog: Any, mock_interaction: Any
    ) -> None:
        ...
        mock_guild.create_text_channel = AsyncMock(
            side_effect=lambda *args, **kwargs: (
                ...
                else mock_create_ch(*args, **kwargs)
            )
        )
    ...
    async def test_setup_existing_sync(
        self,
        mock_count: Any,
        mock_spawn: Any,
        mock_sleep: Any,
    ...
    @patch("mods.revocord.setup.asyncio.sleep", new_callable=AsyncMock)
    @patch("mods.revocord.hunting.initial_wilds_spawn", new_callable=AsyncMock)
    async def test_setup_fetch_fallback(
        self, mock_spawn: Any, mock_sleep: Any, setup_cog: Any, mock_interaction: Any
    ) -> None:
        mock_guild = mock_interaction.guild
        mock_guild.categories = []
    ...
    @pytest.mark.asyncio
    async def test_forbidden_exception(
        self, setup_cog: Any, mock_interaction: Any
    ) -> None:
        mock_guild = mock_interaction.guild
        mock_guild.categories = []
    ...
    async def test_generic_exception(
        ...
        mock_interaction.followup.send.assert_called_with(
            "An error occurred: Random error", ephemeral=True
        )
    ...
    async def test_setup_portal_fail(
        ...
        calls = [
            call
            for call in mock_interaction.followup.send.mock_calls
            if "Portal channel failed to generate." in str(call)
        ]
        assert len(calls) > 0
