...
from mods.revocord.dashboard import DashboardCog, DashboardView, setup
...
class TestDashboardViewCallbacks:
    ...
    async def test_profile_callback(self, mock_build: Any, mock_get_account: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        
        profile_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_profile"][0]
        ...
        mock_interaction.followup.send.assert_called_once_with(view="mocked_view", ephemeral=True)

    @pytest.mark.asyncio
    @patch("mods.revocord.dashboard.get_or_create_account")
    async def test_profile_callback_no_orbs_no_caught(self, mock_get_account: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        profile_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_profile"][0]
    ...
    async def test_profile_callback_exception(self, mock_logger: Any, mock_get_account: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        profile_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_profile"][0]
        ...
        mock_interaction.followup.send.assert_called_with("❌ An error occurred loading your profile: Test Error", ephemeral=True)
        mock_logger.error.assert_called_once()

    @pytest.mark.asyncio
    @patch("mods.revocord.dashboard.update_account")
    async def test_logout_callback(self, mock_update: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        logout_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_logout"][0]
        ...
        mock_update.assert_called_once_with(mock_interaction.user.id, is_logged_in=False)
        mock_portal.set_permissions.assert_called_once_with(mock_interaction.user, overwrite=None)
        mock_other.set_permissions.assert_called_once_with(mock_interaction.user, overwrite=None)
        mock_interaction.followup.send.assert_called_with("✅ You have successfully logged out. You can return via the portal.", ephemeral=True)

    @pytest.mark.asyncio
    @patch("mods.revocord.dashboard.update_account")
    async def test_logout_callback_no_category(self, mock_update: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        logout_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_logout"][0]
        ...
        mock_interaction.followup.send.assert_called_with("❌ Error: Workspace structure not found.", ephemeral=True)

    @pytest.mark.asyncio
    @patch("mods.revocord.dashboard.update_account")
    async def test_logout_callback_forbidden(self, mock_update: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        logout_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_logout"][0]
        ...
        mock_portal = MagicMock(name="portal")
        mock_portal.name = "portal"
        mock_portal.set_permissions = AsyncMock(side_effect=discord.Forbidden(MagicMock(), ''))
        ...
        mock_interaction.followup.send.assert_called_with("❌ Error: I don't have permissions to update your access.", ephemeral=True)

    @pytest.mark.asyncio
    @patch("mods.revocord.dashboard.update_account")
    @patch("mods.revocord.dashboard.logger")
    async def test_logout_callback_exception(self, mock_logger: Any, mock_update: Any, mock_interaction: Any) -> None:
        view = DashboardView()
        logout_button = [x for x in view.children if getattr(x, 'custom_id', '') == "persistent_dashboard_logout"][0]
        ...
        mock_interaction.followup.send.assert_called_with("❌ An unexpected error occurred: Test Error", ephemeral=True)
        mock_logger.error.assert_called_once()
