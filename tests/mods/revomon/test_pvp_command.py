...
class TestPvpLeaderboard2:
    ...
    @patch('mods.revomon.pvp_command.ImageFont.truetype', side_effect=Exception("Font Exception"))
    ...
    @pytest.mark.asyncio
    async def test_pvp_command_success(self, mock_bot: Any, mock_interaction: Any) -> None:
        cog = PvpLeaderboard2(mock_bot)
    ...
    @pytest.mark.asyncio
    async def test_pvp_command_exceptions(self, mock_bot: Any, mock_interaction: Any) -> None:
        cog = PvpLeaderboard2(mock_bot)
        mock_bytes = MagicMock()
        cog.pvp_img = {"image_bytes": mock_bytes}
