...
text = text.replace('assert "embed" in kwargs\n        assert "file" in kwargs', 'assert kwargs.get("embed")\n        assert kwargs.get("attachments")')

# test_spawn_success_not_shiny_no_file
text = text.replace('assert "file" not in kwargs', 'assert not kwargs.get("attachments")')
