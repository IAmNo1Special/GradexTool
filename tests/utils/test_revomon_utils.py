...
@pytest.mark.asyncio
async def test_get_perferred_natures(
    mock_get_natures: Any, mock_get_attributes: Any
) -> None:
...
async def test_get_book_of_mon_names(mock_tables: Any) -> None:
    rt = mock_tables[0]
    rt.return_value.get_names = AsyncMock(
        return_value=["A", "B", "C", "D", "E", "vyphern", "wyverdant", "F"]
    )
...
def test_get_stat_weights() -> None:
    ...
    w, r = get_stat_weights(
        {"base_spd": 120, "base_def": 100, "base_hp": 120, "base_spe": 60}
    )
    assert r == "Defensive Balanced"
    assert w["spd"] == 1.8
    assert w["def"] == 1.2
...
def test_create_graded_mon_img(
    mock_figure: Any, mock_truetype: Any, mock_draw: Any, mock_new: Any, mock_open: Any
) -> None:
...
@patch("utils.revomon_utils.ImageFont.truetype")
@patch("utils.revomon_utils.go.Figure")
def test_create_graded_mon_img_os_error_font_and_image(
    mock_figure: Any, mock_truetype: Any, mock_draw: Any, mock_new: Any, mock_open: Any
) -> None:
    mock_image_instance = MagicMock()
    mock_image_instance.width = 100
...
@patch("utils.revomon_utils.ImageFont.truetype")
@patch("utils.revomon_utils.go.Figure")
def test_create_graded_mon_img_grades(
    mock_figure: Any, mock_truetype: Any, mock_draw: Any, mock_new: Any, mock_open: Any
) -> None:
    mock_image_instance = MagicMock()
    mock_image_instance.width = 100
