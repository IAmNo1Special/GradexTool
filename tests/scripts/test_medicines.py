...
async def test_process_item() -> None:
    ...
    with patch("scripts.medicines.fetch_url", new_callable=AsyncMock) as mock_fetch:
        ...
        await process_item(
            semaphore,
            client,
            {"name": "Item1", "category": "Cat1", "url": "url1"},
            potions_list,
        )
        ...
        await process_item(
            semaphore,
            client,
            {"name": "Item2", "category": "Cat1", "url": "url2"},
            potions_list,
        )
        ...
        await process_item(
            semaphore,
            client,
            {"name": "Item3", "category": "Cat1", "url": "url3"},
            potions_list,
        )
...
async def test_get_medicines(
    mock_json_dump: Any, mock_open_file: Any, mock_makedirs: Any
) -> None:
    with patch(
        "scripts.medicines.get_medicine_categories", new_callable=AsyncMock
    ) as mock_get_cat:
        with patch(
            "scripts.medicines.process_item", new_callable=AsyncMock
        ) as mock_process:
