...
def test_get_evolutions_save_to_file() -> None:
    mock_data = [{"name": "A", "evolution": "B", "levelEvolution": 10}]
    with (
        patch("scripts.evolutions.REVOMON_FILE") as mock_revomon_file,
        ...
        mock_makedirs.assert_called_once_with(
            mock_evolutions_file.parent, exist_ok=True
        )
...
def test_main() -> None:
    with (
        patch("configs.REVOMON_FILE") as mock_revomon,
