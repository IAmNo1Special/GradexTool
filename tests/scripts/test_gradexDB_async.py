...
def mock_json_files() -> Any:
    mock_data = {
        "./data/counterdex.json": json.dumps([{"name": "testmon", "description": "desc", "tier": "A", "metamoves": "none", "metabuilds": "none", "tips": "none", "counters": "none", "weakness": "none"}]),
        "./data/abilities.json": json.dumps({"1": {"id": 1, "name": "overgrow", "effect_entries": [{"short_effect": "Boosts grass."}]}, "2": {"id": 2, "name": "blaze", "flavor_text_entries": [{"flavor_text": "Boosts fire."}]}}),
        "./data/capsules.json": json.dumps([{"name": "capsule1", "description": "captures"}]),
        "./data/fruitys.json": json.dumps({"1": {"idFruity": 1, "name": "apple", "description": "tasty", "type": "food"}}),
        "./data/items.json": json.dumps({"item1": {"name": "potion", "description": "heals", "obtained_from": "shop", "cost": 100}}),
        "./data/natures.json": json.dumps({"nature1": {"name": "adamant", "increased_stat": "atk", "decreased_stat": "spa"}}),
        "./data/revomon.json": json.dumps([{"dex_id": 1, "mon_id": 1, "name": "testmon", "description": "desc", "type1": "fire", "type2": None, "ability1": "blaze", "ability2": None, "ability_hidden": None, "hp": 50, "atk": 50, "def": 50, "spa": 50, "spd": 50, "spe": 50, "evolution": None, "level_evolution": None, "rarity": "common"}]),
...
@pytest.mark.gradexDB
@pytest.mark.asyncio
async def test_moves_table(mock_db: Any, mock_requests: Any, mock_json_files: Any) -> None:
    ...
    with closing(sqlite3.connect(mock_db)) as conn, conn:
        conn.execute("INSERT INTO revomon (dex_id, mon_id, name) VALUES (1, 1, 'testmon')")
        conn.commit()
...
async def test_ownedlands_update_sale_data(mock_db: Any) -> None:
    ...
    await table.create()
    with closing(sqlite3.connect(mock_db)) as conn, conn:
        conn.execute("INSERT INTO ownedLands (token_id, id, owners_address, biome, land_type, rarity, size, img_url, emoji, for_sale) VALUES (1, 1, '0x', 'f', 'b', 'r', 's', 'u', 'e', 0)")
        conn.execute("INSERT INTO ownedLands (token_id, id, owners_address, biome, land_type, rarity, size, img_url, emoji, for_sale) VALUES (2, 2, '0x', 'f', 'b', 'r', 's', 'u', 'e', 0)")
        conn.commit()

    with patch("scripts.gradexDB.OwnedLandsTable.get_ids") as m_ids, \
         patch("scripts.gradexDB.get_lands_for_sale_amount", new_callable=AsyncMock) as m_sale_data:
        m_ids.return_value = [1, 2]
        m_sale_data.return_value = {"1": {"owners_address": "0x2", "for_sale_token": 10, "token_symbol": "ETH", "for_sale_usd": 20}}
...
@pytest.mark.gradexDB
@pytest.mark.asyncio
async def test_revomon_moves_table(mock_db: Any, mock_requests: Any, mock_json_files: Any) -> None:
    rev_table = RevomonTable()
    await rev_table.create()
    ...
    with closing(sqlite3.connect(mock_db)) as conn, conn:
        conn.execute("INSERT INTO revomon (dex_id, mon_id, name) VALUES (1, 1, 'testmon')")
        conn.execute("INSERT INTO moves (id, name, category, type, description, accuracy, power, pp, priority) VALUES (1, 'testmove', 'cat', 'type', 'desc', 100, 40, 35, 1)")
        conn.commit()
