...
async def test_get_caught_revomon_200_valid() -> None:
    ...
    req = Request("GET", "http://test")
    resp = Response(
        200, json={"data": {"catchedRevomon": {"name": "TestMon"}}}, request=req
    )
...
def test_save_progress() -> None:
    ...
    state = {"next_id": 2, "statuses": {"1": "found", "2": "not_found", "3": "error"}}
...
async def test_process_batch() -> None:
    ...
    async def mock_get(
        client: Any, pacer: Any, id_catched: Any, max_attempts: Any = 8
    ) -> Any:
...
async def test_get_caught_revomon_data_max_id() -> None:
    with (
        patch("scripts.caught_revomon._load_results", return_value={}),
        patch(
            "scripts.caught_revomon._load_scan_state",
            return_value={"version": 1, "next_id": 1, "statuses": {}},
        ),
        patch(
            "scripts.caught_revomon._process_batch", new_callable=AsyncMock
        ) as mock_batch,
        patch("scripts.caught_revomon.save_progress") as mock_save,
    ):
        mock_batch.side_effect = lambda c, p, ids: (
            [CaughtRevomonResult("not_found")] * len(ids)
        )
...
async def test_get_caught_revomon_data_empty_tail() -> None:
    with (
        patch("scripts.caught_revomon._load_results", return_value={1: {"name": "A"}}),
        patch(
            "scripts.caught_revomon._load_scan_state",
            return_value={"version": 1, "next_id": 2, "statuses": {"1": "found"}},
        ),
        patch(
            "scripts.caught_revomon._process_batch", new_callable=AsyncMock
        ) as mock_batch,
...
async def test_get_caught_revomon_data_error_pause() -> None:
    with (
        ...
        mock_batch.side_effect = lambda c, p, ids: (
            [CaughtRevomonResult("error")] * len(ids)
        )
        results = await get_caught_revomon_data(max_id=1, empty_tail_limit=10)
        assert results == {}
...
async def test_get_caught_revomon_data_found() -> None:
    with (
        ...
        def mock_side_effect(c: Any, p: Any, ids: Any) -> Any:
            return [
                CaughtRevomonResult("found", {"name": "Test"})
                if i == 1
                else CaughtRevomonResult("not_found")
                for i in ids
            ]
...
async def test_get_caught_revomon_data_existing_status() -> None:
    with (
        ...
        mock_batch.side_effect = lambda c, p, ids: [
            CaughtRevomonResult("found", {"name": "B"})
            if i == 3
            else CaughtRevomonResult("not_found")
            for i in ids
        ]
...
async def test_get_caught_revomon_data_100() -> None:
    with (
        ...
        patch(
            "scripts.caught_revomon._load_scan_state",
            return_value={"version": 1, "next_id": 100, "statuses": {}},
        ),
        patch(
            "scripts.caught_revomon._process_batch", new_callable=AsyncMock
        ) as mock_batch,
...
def test_main_execution() -> None:
    with patch("asyncio.run", side_effect=lambda coro: coro.close()) as mock_run:
        ...
        with patch.dict("sys.modules"):
            ...
            try:
                runpy.run_module(
                    "scripts.caught_revomon", run_name="__main__", alter_sys=True
                )
            except Exception:
                pass
