...
for conv_id in os.listdir(brain_dir):
    transcript_path = os.path.join(
        brain_dir, conv_id, ".system_generated", "logs", "transcript_full.jsonl"
    )
    if os.path.exists(transcript_path):
                    ...
                    if "tool_calls" in data:
                        for call in data["tool_calls"]:
                            if call["function"] == "default_api:write_to_file":
                                args = call["arguments"]
                                if "TargetFile" in args and args["TargetFile"].endswith(
                                    "test_hunting.py"
                                ):
        ...
        if latest_content:
            ...
            with open(
                r"f:\projects\Revomon\GradexTool\tests\mods\revocord\test_hunting.py",
                "w",
                encoding="utf-8",
            ) as out:
